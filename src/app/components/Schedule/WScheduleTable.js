...
import {
  ...
  saveAction,
  closeNotifAction,
} from '../../actions/schedule/WScheduleActions';
...
// Reducer branch
const branch = 'wscheduleTable';

class WScheduleTable extends Component {
      ...
      return (
        <div>
          ...
          <Paper className={classes.root}>
            <CrudTable
              dataInit={content}
              anchor={wscheduleConst.table.columns}
              title={wscheduleConst.table.title}
              dataTable={content}
              fetchData={fetchData}
  ...
  const WScheduleTableMapped = connect(
    mapStateToProps,
    mapDispatchToProps
  )(WScheduleTable);
  
  export default withStyles(styles)(WScheduleTableMapped);